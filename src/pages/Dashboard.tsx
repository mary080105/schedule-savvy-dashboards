...
const Dashboard = () => {
  ...
  // This would be generated by the backend
  const [durationHistogramUrl, setDurationHistogramUrl] = useState("");
  const [startTimeBarChartUrl, setStartTimeBarChartUrl] = useState("");
  const [nameFrequencyPieChartUrl, setNameFrequencyPieChartUrl] = useState("");
  ...
  const loadScheduleData = (schedule: string) => {
    ...
    // Generate chart URLs (in a real app, these would come from the backend)
    setDurationHistogramUrl(`https://via.placeholder.com/600x400?text=Duration+Histogram+for+${encodeURIComponent(schedule)}`);
    setStartTimeBarChartUrl(`https://via.placeholder.com/600x400?text=Start+Time+Chart+for+${encodeURIComponent(schedule)}`);
    setNameFrequencyPieChartUrl(`https://via.placeholder.com/600x400?text=Event+Names+for+${encodeURIComponent(schedule)}`);
  };

  const greedySchedule = (events: Event[]): Event[] => {
  ...
  const calculateAnalytics = (events: Event[]): Analytics => {
    if (!events.length) {
      return {
        total_events: 0,
        avg_duration_minutes: 0,
              ...
              <div className="px-4 py-5 sm:p-6">
                <div className="space-y-3 overflow-y-auto max-h-80">
                  {events.length === 0 ? (
                    ...
                    events.map(event => (
                      <div key={event.id} className="bg-white rounded-lg shadow-md p-4 mb-3 hover:shadow-lg transition-shadow">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-white shadow rounded-lg transition-transform hover:transform hover:scale-105">
                ...
                <div className="px-4 py-5 sm:p-6 h-64 flex items-center justify-center">
                  {analytics.total_events > 0 ? (
                    <img src={durationHistogramUrl} alt="Duration Histogram" className="max-h-full max-w-full object-contain" />
                  ) : (
              ...
              <div className="bg-white shadow rounded-lg transition-transform hover:transform hover:scale-105">
                ...
                <div className="px-4 py-5 sm:p-6 h-64 flex items-center justify-center">
                  {analytics.total_events > 0 ? (
                    <img src={startTimeBarChartUrl} alt="Start Time Bar Chart" className="max-h-full max-w-full object-contain" />
                  ) : (
                    <p className="text-gray-400">No data available</p>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white shadow rounded-lg transition-transform hover:transform hover:scale-105">
                ...
                <div className="px-4 py-5 sm:p-6 h-64 flex items-center justify-center">
                  {analytics.total_events > 0 ? (
                    <img src={nameFrequencyPieChartUrl} alt="Event Name Pie Chart" className="max-h-full max-w-full object-contain" />
                  ) : (
                    <p className="text-gray-400">No data available</p>
                  )}
                </div>
                ...
                <div className="px-4 py-5 sm:p-6">
                  <div className="space-y-3 overflow-y-auto max-h-72">
                    {optimizedEvents.length === 0 ? (
                      <p className="text-gray-500 text-center py-4">No optimized events available</p>
                    ) : (
                      optimizedEvents.map(event => (
                        <div key={`opt-${event.id}`} className="bg-white rounded-lg shadow-md p-4 mb-3 border-l-4 border-green-500">
                          <h3 className="font-semibold text-gray-800 text-lg">{event.name}</h3>
                          <div className="flex flex-col sm:flex-row sm:justify-between mt-2 text-sm">
                            <div className="text-gray-600">
                              <span className="inline-flex items-center">
